  ...
  var VM = {
    ...
    validators: {
      ...
      format: function(value, options) {
        var msgs = [];
        if ('with' in options) {
          var withReg = new RegExp(VM.utils.convertRegExp(options.with).regExp, 'g');
          console.log(withReg);
          if (!withReg.test(value)) msgs.push(options.message || VM.messages.invalid);
        };
        if ('without' in options) {
          var withoutReg = new RegExp(VM.utils.convertRegExp(options.without).regExp, 'g');
