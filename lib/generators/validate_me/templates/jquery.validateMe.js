(function($, window, document) {
  ...
  $.fn.extend({
    ...
    validate: function(eventType, userCallbacks) {
      
      eventType = eventType || 'keyup';
      callbacks = VM.defaultCallbacks;
  ...
  var VM = {
    ...
    validators: {
      ...
      format: function(value, options) {
        var msgs = [];
        if ('with' in options) {
          var withReg = VM.utils.convertRegExp(options.with);
          console.log(withReg);
          if (!withReg.test(value)) msgs.push(options.message || VM.messages.invalid);
        };
        if ('without' in options) {
          var withoutReg = VM.utils.convertRegExp(options.without);
          console.log(withoutReg)
          if (withoutReg.test(value)) msgs.push(options.message || VM.messages.invalid);
        };
        return msgs.length? msgs : false;
      }
    ...
    utils: {
      ...
      convertRegExp: function(string) {
        var p = string.slice(1, -1).split(':'),
            o = p.shift(),
            r = p.join(':');
        return new RegExp(r, VM.utils.convertOptions(o));
      },
