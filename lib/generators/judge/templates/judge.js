...
judge.Watcher = function (element) {
  ...
  if (!judge.utils.isValidatable(element)) {
    throw new TypeError('[judge] Cannot construct new Watcher instance from object of this type');
  }
...
judge.store = (function() {
  ...
  return {
    ...
    // get all stored DOM elements, or all DOM elements stored within all Watchers stored against key
    getDOM: function(key) {
      if (_(key).isUndefined()) {
        return _(store).chain().values().flatten().pluck('element').value();
      }
      return judge.store.get(key) === null ? null : _(judge.store.get(key)).pluck('element');
    },
...
judge.validate = function(elements) {
  var results = [];
  elements = judge.utils.isCollection(elements) ? elements : judge.stored;
  _(elements).each(function(element) {
