...
var Judge = {
  ...
  Validators: {
    ...
    exclusion: function(value, options) {
      return $.inArray(value, $.map(options.in, function(i) { return i.toString(); })) == -1 ? false : [options.message || Judge.DefaultErrors.exclusion];
    },

    inclusion: function(value, options) {
      return $.inArray(value, $.map(options.in, function(i) { return i.toString(); })) > -1 ? false : [options.message || Judge.DefaultErrors.inclusion];
    },
    ...
    numericality: function(value, options) {
      ...
      if (isNaN(Number(value))) {
        ...
        $.each(operators, function(key, operator) {
          if (key in options && !Judge.Utilities.check(parsedValue, operators[key], parseFloat(options[key])))
  ...
  Utilities: {
    ...
    isOdd: function(value) { return !Judge.Utilities.isEven(value); },
    ...
    convertRegExp: function(string) {
      var p = string.slice(1, -1).split(':'),
          o = p.shift(),
          r = p.join(':');
      return new RegExp(r, Judge.Utilities.convertFlags(o));
    },
